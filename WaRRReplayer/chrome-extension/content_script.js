...
console.log("BLA: " + ChromeDriverContentScript.currentDocument.documentOwner);

if (ChromeDriverContentScript.currentDocument.location != "about:blank" || ChromeDriverContentScript.currentDocument.documentOwner != undefined) {
  ...
  console.log(">>SILVIU [22] " + ChromeDriverContentScript.currentDocument.location);
  var frameName = ChromeDriverContentScript.currentDocument.location != "about:blank" ? window.name : (window.name+"_inner_frame");
  ChromeDriverContentScript.port = chrome.extension.connect({name: frameName});
...
function parsePortMessage(message) {
  ...
  try {
    ...
    case "sniffForMetaRedirects":
      ...
      break;
    case "dragElement":
      dragElement(element, message.request.x, message.request.y);
      response.value={statusCode: "no-op"};
      response.wait = false;
      break;
    default:
...
function dispatchMouseEvent(event, target, clientX, clientY) {
  var evt = target.ownerDocument.createEvent("MouseEvents");
  evt.initMouseEvent(event, true, true, target.ownerDocument.defaultView, 1, 0, 0, clientX, clientY, false, false,false, false, 0, target);
...
function dragElement(element, x, y){
  ...
  coordsBody = getElementCoords(element.ownerDocument.body);
  dispatchMouseEvent("mousedown", element, 0, 0);
  dispatchMouseEvent("mousemove", element.parentElement, coords[0]-coordsBody[0]+x, coords[1]-coordsBody[1]+y);
  dispatchMouseEvent("mouseup", element.parentElement, coords[0]-coordsBody[0]+x, coords[1]-coordsBody[1]+y);
}
...
function dispatchKeyboardEvent(event, key,target) {
  var evt = ChromeDriverContentScript.currentDocument.createEvent("KeyboardEvent");
  evt.initKeyboardEvent (event, true, true, window, key, "") 
  evt.charCode = key.charCodeAt(0);
  target.dispatchEvent(evt);
}

function typeKey(key, target, property){
  dispatchKeyboardEvent("keydown", key, target);
  dispatchKeyboardEvent("keypress", key, target);
  dispatchKeyboardEvent("textInput", key, target);
  target[property] += key;
  dispatchKeyboardEvent("keyup", key, target);
}
...
function sendElementNonNativeKeys(element, keys) {
  //Silviu: modeified to support the necessary key events
 /* var script=
'function dispatchKeyboardEvent(event, key,target) {\n'+
...
'}\n'+
  ...
  '  typeKey(keys.charAt(i), element, property);\n'+
  '}\n';

  var args = [{'ELEMENT': addElementToInternalArray(element)}, keys];
  execute(script, args);*/
  var property;
  if(element.tagName=="DIV"){
    property="textContent";
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    /*element.textContent+=keys.charAt(i);
  }
  console.log("[671]>>> element: "+element.tagName+" is contenteditable? = " + element.isContentEditable);
  element.tagName=="DIV"
    for (var i = 0; i < keys.length; i++) {
        element.textContent+=keys.charAt(i);
    }
 ...
 */
function execute_(script, passedArgs, asyncTimeout, callback, callBackInCaseOfTimeout) {
  ...
  var scriptLoadTimeout = window.setTimeout(function() {
    ...
    if(callBackInCaseOfTimeout){
        callBackInCaseOfTimeout();
        return;
    }
...
function switchToFrame(id) {
  ...
  if(id=="WaRRReset"){
      ChromeDriverContentScript.currentDocument = document;
      return;
  }
  ...
  var findFrame = function(id) {
    function getFrameIndex(element) {
	  ...
	  if(!src){
	       console.log("STUPID FRAME WITHOUT ANY SRC");
	       //ChromeDriverContentScript.currentDocument = frame.target.ownerDocument;
	       console.log("CHANGED CURRENT DOCUMENT TO THE THE WITHIN DOC");
	       return -1;
	  }
          return i;
        }
  ...
  var callBackFunction = function(response) {
    ...
    if(response.value==-1){
       var useID = id['ELEMENT'] ? id['ELEMENT'] : id
       var frame = internalGetElement(useID);
       ChromeDriverContentScript.currentDocument = frame.contentDocument;
       ChromeDriverContentScript.port.postMessage({
        sequenceNumber: ChromeDriverContentScript.currentSequenceNumber,
        response: {
          response: 'switchToFrame',
  ...
  execute_('return (' + findFrame + ').apply(null, arguments);', [id], -1, callBackFunction);
}
...
function getElementsByXPath(xpath, context) {
  ...
  var elements = [];
  //console.log("[XPATH SILVIU] " + xpath+" "+new XMLSerializer().serializeToString(context) + " " + XPathResult.ORDERED_NODE_ITERATOR_TYPE);

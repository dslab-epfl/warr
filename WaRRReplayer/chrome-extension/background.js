...
chrome.extension.onConnect.addListener(function(port) {
  ...
  console.log("Connected to " + port.name + " (" + port.portId_ + ")");
  ...
  port.onDisconnect.addListener(function disconnectPort(port) {
    console.log("Disconnected from " + port.name + " portID_("+port.portId_+")");
    var remainingTabs = [];
    for (var tab in ChromeDriver.tabs) {
      if (ChromeDriver.tabs[tab].tabId == port.tab.id) {
...
function parseRequest(request) {
  ...
  case "executeScript":
    ...
    if(request.script.indexOf("window.resizeTo(") === 0){
      var opt = {};
      var theMatch = request.script.match(/window.resizeTo\((\d+),(\d+)\);/);
      opt.width   = parseInt(theMatch[1]);
      opt.height  = parseInt(theMatch[2]);
      chrome.windows.getCurrent(function (win){
        opt.left = win.left;
        opt.top = win.top;
        chrome.windows.update( win.id, opt, function(dontCare){
          sendResponseToParsedRequest({status: 0, value: null});
        });
      });
     }
...
function parsePortMessage(message, port) {
  ...
  case "no-op":
    ...
    case "newTabInformation":
      ...
      console.log("port.tab.id: " + port.tab.id + ", defaultContent: " + response.isDefaultContent);
      for (var tab in ChromeDriver.tabs) {
        if (ChromeDriver.tabs[tab].tabId == port.tab.id) {
      ...
      if (ChromeDriver.doFocusOnNextOpenedTab && response.isDefaultContent) {
        ChromeDriver.activePort = port;
        console.log(">>>[705] Setting ChromeDriver.activePort to " + port+ " portID = "+port.portId_);
    ...
    case "switchToFrame":
      ...
      ChromeDriver.activePort = port;
      console.log(">>>[733] Setting ChromeDriver.activePort to " + ChromeDriver.activePort);
      sendResponseToParsedRequest({status: 0}, false);
      break;
    default:
      console.log(">>[737] Replying to " + message.response.response +" as default");
      sendResponseToParsedRequest({status: 0}, false);
    }
    break;
  }
...
function switchToFrame(id) {
  ...
  if(id == "WaRRResetFrame") {
    console.log("SILVIU RESSETING THE CURRENT FRAME");
  ...
  if (id === undefined || id === null || id == "WaRRResetFrame") {
    ...
    if(firstTab){
      ChromeDriver.activePort = firstTab;
      console.log(">>>[820] Setting ChromeDriver.activePort to " + ChromeDriver.activePort+ " portID = "+ChromeDriver.activePort.portId_);
      sendResponseToParsedRequest({status: 0}, false);
      console.log("Selected: " + firstTab);
    }
...
function setActivePortByWindowName(handle) {
  for (var tab in ChromeDriver.tabs) {
    if (ChromeDriver.tabs[tab].windowName == handle || 
      ...
      ChromeDriver.activePort = ChromeDriver.tabs[tab].mainPort;
      console.log(">>>[957] Set active port to "+ChromeDriver.tabs[tab].mainPort);
 ...
 */
function hasNoPage() {
  //console.log(ChromeDriver.hasNoConnectionToPage + " "+ChromeDriver.activePort+" "+ChromeDriver.activeTabId);
